...
class MemoryCadenceRepo {
    ...
    await _upsert(
      MemoryCadence(
        ...
        assistantMessagesSinceLastRun:
            current.assistantMessagesSinceLastRun + 1,
  ...
  Future<bool> shouldRun(String sessionId, String kind, int interval) async {
    final current = await get(sessionId);
  ...
  Future<void> _upsert(MemoryCadence cadence) {
    return db
        .into(db.memoryCadenceRows)
        .insertOnConflictUpdate(
          MemoryCadenceRowsCompanion.insert(
            chatSessionId: cadence.chatSessionId,
            assistantMessagesSinceLastRun: Value(
              cadence.assistantMessagesSinceLastRun,
            ),
