...
async function openMemoryGenerationSettings() {
    ...
    const renderSheet = () => {
        const content = document.createElement('div');
        content.className = 'context-sheet';
        const currentEndpointLabel = currentApiConfig.apiType === 'anthropic'
            ? 'api.anthropic.com'
            : (currentApiConfig.apiUrl || 'Not configured');
...
function startGeneration(char, text, existingMsgIndex = -1, onAbort = null, guidanceText = null, guidanceType = 'GENERATION') {
    ...
    const apiType = localStorage.getItem('gz_api_type') || 'openai';

    if (!model || (apiType !== 'anthropic' && !endpoint)) {
        showBottomSheet({
            bigInfo: {
