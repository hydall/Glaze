...
class Bridge {
  ...
  _extBlockImageGenPrompt(instruction) {
    let raw = String(instruction || '').trim();
    if (!raw) return '';
    // Pending payloads that carried images through a regeneration spell them
    // `@paths|instruction`; the instruction is the last `|`-separated segment.
    if (raw.startsWith('@')) {
      const pipeIdx = raw.indexOf('|');
      raw = pipeIdx === -1 ? '' : raw.substring(pipeIdx + 1).trim();
    }
    if (!raw) return '';
    try {
  ...
  _fillExtBlockBody(body, block) {
    ...
    const hasImgGen = imgGenRegex.test(block.content);
