...
void main() {
  ...
  testWidgets('borderOnTop keeps the frame visible over a full-bleed child', (
    ...
    // Same surface without the flag: the child owns that pixel. The film-grain
    // overlay tints it by a hair, so assert the hue rather than the exact byte —
    // the child is red, the frame is blue.
    final bare = await edgePixel(borderOnTop: false);
    expect(bare, isNot(borderColor));
    expect(bare.r, greaterThan(bare.b));
  });
}
