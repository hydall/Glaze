...
void runDbMigrationTests(int shardIndex) {
  ...
  group('Backup importer schema safety', () {
    ...
    setUp(() {
      ...
      db = _testDb();
      imageDirectory = Directory.systemTemp.createTempSync('glaze_test_img_');
      imageStorage = _TestImageStorage(imageDirectory);
    });
  ...
  group('Database migrations', () {
    ...
    test('v118 defaults prompt post-processing without a preset', () async {
      expect(await _upgradeV118With(const {}, null), {
        'custom': 'none',
        'anthropic': 'none',
      });
    });

    test('v118 ignores an inactive legacy merge setting', () async {
      expect(
        await _upgradeV118With(const {
    ...
    test('v118 does not carry over an active legacy merge setting', () async {
      expect(
        await _upgradeV118With(const {
          'activePresetId': 'preset',
    ...
    test('v118 ignores a dangling active preset id', () async {
      expect(
        await _upgradeV118With(const {'activePresetId': 'missing'}, null),
        {'custom': 'none', 'anthropic': 'none'},
      );
    });

    test('v118 preserves an existing prompt post-processing choice', () async {
      expect(await _upgradeV118With(const {}, null, removeColumn: false), {
        'custom': 'single',
        'anthropic': 'none',
      });
    });
    ...
    test('v123 raises only the legacy Studio final history limit', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_studio_history_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customSelect('SELECT 1').get();
      await seeded.customStatement(
        "UPDATE studio_preset_rows SET max_final_history_messages = 30 "
        "WHERE preset_id = 'default'",
      );
      await seeded.customStatement(
        "INSERT INTO studio_preset_rows "
        "(preset_id, name, max_final_history_messages, updated_at) "
        "VALUES ('custom', 'Custom', 42, 0)",
      );
      await seeded.customStatement('PRAGMA user_version = 122');
      await seeded.close();
      ...
      final rows = await upgraded
          .customSelect(
            'SELECT preset_id, max_final_history_messages '
            'FROM studio_preset_rows WHERE preset_id IN (?, ?)',
            variables: [
      ...
      final limits = {
        for (final row in rows)
          row.read<String>('preset_id'): row.read<int>(
            'max_final_history_messages',
          ),
      };

      expect(limits, {'default': 50, 'custom': 42});
      final version = await upgraded
          .customSelect('PRAGMA user_version')
          .getSingle();
      expect(version.read<int>('user_version'), 135);
    ...
    test('v124 and v125 add LLM capture history and linkage', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_llm_capture_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customSelect('SELECT 1').get();
      await seeded.customStatement('DROP TABLE llm_request_capture_rows');
      await seeded.customStatement('PRAGMA user_version = 123');
      await seeded.close();
      ...
      addTearDown(() async => upgraded.close());
      final columns = await upgraded
          .customSelect("PRAGMA table_info('llm_request_capture_rows')")
          .get();
      final indexes = await upgraded
          .customSelect("PRAGMA index_list('llm_request_capture_rows')")
          .get();
      final version = await upgraded
          .customSelect('PRAGMA user_version')
          .getSingle();
      ...
      expect(
        ...
        containsAll([
          ...
          'idx_llm_request_capture_created',
          'idx_llm_request_capture_call',
        ]),
      );
      final eventColumns = await upgraded
          .customSelect("PRAGMA table_info('llm_call_event_rows')")
          .get();
      final eventIndexes = await upgraded
          .customSelect("PRAGMA index_list('llm_call_event_rows')")
          .get();
      expect(
      ...
      expect(
        eventIndexes.map((row) => row.read<String>('name')),
        containsAll([
          'idx_llm_call_event_session_created',
    ...
    test('v125 upgrades an existing v124 request capture table', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_llm_events_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customSelect('SELECT 1').get();
      await seeded.customStatement('DROP TABLE llm_call_event_rows');
      await seeded.customStatement('DROP TABLE llm_request_capture_rows');
      await seeded.customStatement('''
        CREATE TABLE llm_request_capture_rows (
          ...
          logical_call_id TEXT NULL,
          related_artifact_id TEXT NULL,
          agent_id TEXT NULL,
          stage_ordinal INTEGER NULL,
      ...
      ''');
      await seeded.customStatement('PRAGMA user_version = 124');
      await seeded.close();
      ...
      addTearDown(() async => upgraded.close());
      final requestColumns = await upgraded
          .customSelect("PRAGMA table_info('llm_request_capture_rows')")
          .get();
      final eventColumns = await upgraded
          .customSelect("PRAGMA table_info('llm_call_event_rows')")
      ...
      expect(
        requestColumns.map((row) => row.read<String>('name')),
        contains('call_id'),
      );
    ...
    test('v126 adds immutable reconciliation effects', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_reconciliation_effects_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customSelect('SELECT 1').get();
      await seeded.customStatement('DROP TABLE ledger_reconciliation_effects');
      await seeded.customStatement('PRAGMA user_version = 125');
      await seeded.close();
      ...
      addTearDown(() async => upgraded.close());
      final columns = await upgraded
          .customSelect("PRAGMA table_info('ledger_reconciliation_effects')")
          .get();
      final indexes = await upgraded
          .customSelect("PRAGMA index_list('ledger_reconciliation_effects')")
          .get();
      final version = await upgraded
          .customSelect('PRAGMA user_version')
          .getSingle();
      ...
      expect(
        ...
        containsAll([
          ...
          'before_ledger_json',
          'after_ledger_json',
          'before_knowledge_json',
          'after_knowledge_json',
          'actual_effects_json',
          'before_state_hash',
          'after_state_hash',
          'effects_hash',
    ...
    test(
      'v127 preserves collector rows and adds failed recovery fields',
      () async {
        final file = File(
          '${Directory.systemTemp.path}/glaze_mig_collector_failure_${DateTime.now().microsecondsSinceEpoch}.db',
        );
        addTearDown(() async {
          if (file.existsSync()) await file.delete();
        });
        ...
        await seeded.customStatement(
          ...
          'RENAME TO card_evolution_collector_runs_v127',
        );
        await seeded.customStatement('''
CREATE TABLE card_evolution_collector_runs (
  id TEXT NOT NULL PRIMARY KEY,
  session_id TEXT NOT NULL,
  character_id TEXT NOT NULL,
  collector_ordinal INTEGER NOT NULL,
  reconciliation_run_id TEXT NOT NULL,
  reconciliation_run_ordinal INTEGER NOT NULL,
  reconciliation_chain_hash TEXT NOT NULL,
...
''');
        await seeded.customStatement('''
INSERT INTO card_evolution_collector_runs VALUES
('collector', 'session', 'character', 1, 'run', 2, 'chain', 'range',
 'input', 'owner', 'claimed', 50, NULL, 1, NULL)
''');
        await seeded.customStatement(
          'DROP TABLE card_evolution_collector_runs_v127',
        );
        await seeded.customStatement('PRAGMA user_version = 126');
        await seeded.close();
    ...
    test('v128 preserves claims and adds writer call checkpoints', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_writer_calls_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customStatement(
        ...
        'RENAME TO card_evolution_claims_v128',
      );
      await seeded.customStatement(
      ...
      await seeded.customStatement('''
CREATE TABLE card_evolution_claims (
  id TEXT NOT NULL PRIMARY KEY,
  session_id TEXT NOT NULL,
  character_id TEXT NOT NULL,
  owner_id TEXT NOT NULL,
  ...
  predecessor_cursor_hash TEXT NOT NULL,
  predecessor_run_ordinal INTEGER NOT NULL,
  input_hash TEXT NOT NULL,
  rewrite_job_id TEXT,
  created_at INTEGER NOT NULL,
  completed_at INTEGER,
...
''');
      await seeded.customStatement('''
INSERT INTO card_evolution_claims VALUES
('claim', 'session', 'character', 'owner', 'claimed', 50, 'history',
 'canon', 'cursor', 2, 'input', NULL, 1, NULL)
''');
      await seeded.customStatement('DROP TABLE card_evolution_claims_v128');
      await seeded.customStatement('PRAGMA user_version = 127');
      await seeded.close();
      ...
      addTearDown(() async => upgraded.close());
      final claimColumns = await upgraded
          .customSelect("PRAGMA table_info('card_evolution_claims')")
          .get();
      ...
      expect(
        claimColumns.map((column) => column.read<String>('name')),
        containsAll([
          'selected_input_json',
          'writer_options_json',
          'failure_code',
    ...
    test('v130 adds constrained reconciliation leases', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_lease_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customSelect('SELECT 1').get();
      await seeded.customStatement('DROP TABLE ledger_reconciliation_leases');
      await seeded.customStatement('PRAGMA user_version = 129');
      await seeded.close();
    ...
    test('v132 resets pair-based Collector state only', () async {
      final file = File(
        '${Directory.systemTemp.path}/glaze_mig_collector_batch_${DateTime.now().microsecondsSinceEpoch}.db',
      );
      addTearDown(() async {
        if (file.existsSync()) await file.delete();
      });
      ...
      await seeded.customStatement(
        ...
        "input_hash, owner_id, status, lease_expires_at, created_at) VALUES "
        "('collector', 'session', 'character', 1, 'run-2', 2, 'chain', "
        "'pair-range', 'collector-input', 'owner', 'completed', 0, 1)",
      );
      await seeded.customStatement(
        "INSERT INTO card_evolution_observations "
        "(id, session_id, character_id, run_ordinal, semantic_scope_key, "
        ...
        "('observation', 'session', 'character', 1, 'scope', 'change', "
        "'[]', 0.5, 'active', 1, 1, 1)",
      );
      await seeded.customStatement(
        ...
        "('pending', 'session', 'character', 'owner', 'failed', 0, 'history', "
        "'canon', 'chain', 1, 'pending-input', 1), "
        "('completed', 'session', 'character', 'owner', 'completed', 0, "
        "'history', 'canon', 'chain', 1, 'completed-input', 1)",
      );
      await seeded.customStatement(
        "INSERT INTO card_evolution_writer_calls "
        "(id, claim_id, session_id, ordinal, stage, stage_ordinal, status, "
        "prompt, prompt_hash, failure_code, created_at, updated_at, failed_at) "
        "VALUES ('call', 'pending', 'session', 1, 'card_writer', 1, 'failed', "
        "'prompt', 'hash', 'parserRejected', 1, 1, 1)",
      );
      await seeded.customStatement('PRAGMA user_version = 131');
      await seeded.close();
      ...
      expect(
        await upgraded.select(upgraded.cardEvolutionCollectorRuns).get(),
        isEmpty,
      );
      expect(
        await upgraded.select(upgraded.cardEvolutionObservations).get(),
        isEmpty,
      );
      expect(
        (await upgraded.select(upgraded.cardEvolutionClaims).get()).single.id,
        'completed',
      );
      expect(
        await upgraded.select(upgraded.cardEvolutionWriterCalls).get(),
        isEmpty,
      );
    });
  });
...
Future<Map<String, String>> _upgradeV118With(
  ...
  for (final config in const [
    ...
    await seeded.customStatement(
      'INSERT INTO api_configs (config_id, name, protocol) VALUES (?, ?, ?)',
