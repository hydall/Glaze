...
| `ElevatedButton` / `OutlinedButton` / `FilledButton` in a toolbar | `GlassSurface` tile with `onTap`, `GlazePillButton` (`glaze_scaffold.dart`), or `GlazeActionButton` (`glaze_action_button.dart`) | There is no generic Glaze button — a tile built on `GlassSurface` *is* the button. `GlazeActionButton` is that tile with the states an action needs: `tone` (primary / neutral / destructive), a null `onTap` for disabled, `busy` for a spinner in place of the glyph, `expand` for a full-width bar. Reach for it where a surface has several actions that must read as one control. |
| `ExpansionTile` | `GlazeExpansionTile` (`glaze_expansion_tile.dart`) | A header that opens a body, in the kit's colours and haptics. `surface: true` gives it its own glass card; leave it false inside a `GlassSurface` or `MenuGroup` so the two do not stack. |
| `Chip` / `FilterChip` / `ChoiceChip` | `GlazeFilterChipBar` (`glaze_filter_chip_bar.dart`), `GlazeDropdownChip` / `GlazeActionChip` / `GlazeFilterIconButton` (`list_controls.dart`), `CardTagChips`, `VariationChip`, `ConnectionChip` / `ConnectionScopeChip` | Pick the one that matches the role; a bare toggle in a form is usually a `MenuSwitchItem`, not a chip. |
