...
* `assets/chat_webview/renderer/shadow_document_shim.js` — widens `document.getElementById` / `querySelector` / `querySelectorAll` so the JS a message writes reaches its own DOM: the message root whose script is running (or whose element an event is being dispatched to) is consulted first, then the real document, then every message shadow root. Installed lazily by `executeInlineScripts`, so a chat that never runs message JS keeps the untouched methods (INV-JS9)
