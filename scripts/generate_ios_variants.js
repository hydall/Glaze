...
async function generateVariants() {
    try {
        const darkSvgStr = `
            <svg width="1024" height="1024" viewBox="0 0 1200 1200" xmlns="http://www.w3.org/2000/svg">
                ...
                <g transform="translate(300, 900) scale(0.1, -0.1)">
                    <path fill="white" stroke="white" stroke-width="150" stroke-linejoin="round" d="${pathD}"/>
                </g>
            </svg>
        `;
        ...
        const tintedSvgStr = `
            <svg width="1024" height="1024" viewBox="0 0 1200 1200" xmlns="http://www.w3.org/2000/svg">
                <!-- For tinted, we use transparent background and white shape -->
        ...
        await sharp(Buffer.from(darkSvgStr))
            .png()
            .toFile(path.join(baseDir, 'AppIcon-Dark.png'));
            
        await sharp(Buffer.from(tintedSvgStr))
            .png()
            .toFile(path.join(baseDir, 'AppIcon-Tinted.png'));

        // Update Contents.json
        const contentsPath = path.join(baseDir, 'Contents.json');
        const contents = JSON.parse(fs.readFileSync(contentsPath, 'utf8'));

        // Check if dark/tinted already exist
        const hasDark = contents.images.some(img => img.appearances && img.appearances[0].value === 'dark');
        ...
        console.log('Successfully generated Dark and Tinted variants for iOS.');
    } catch (err) {
